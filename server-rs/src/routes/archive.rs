...
async fn get_subject(
    ...
    let result = db::with_archive_db(Arc::clone(&pools), move |conn| {
        let raw: Option<String> = conn
            .query_row(
                "SELECT raw_json FROM subjects WHERE id = ?1",
                [id],
    ...
    match result {
        ...
        Ok(None) => {
            ...
            match super::bgm_get(&url).await {
                Ok(v) => {
                    super::cache_put_ttl(&super::SUBJECT_DETAILS_CACHE, cache_key, 10 * 60 * 1000, v.clone());
                    Json(v).into_response()
                }
    ...
    if char_val_opt.is_none() {
        ...
        match super::bgm_get(&url).await {
            Ok(raw) => {
                ...
                let image = raw.get("images")
                    .and_then(|imgs| imgs.get("medium").or_else(|| imgs.get("large")).or_else(|| imgs.get("grid")))
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string())
                    .unwrap_or_else(|| "https://lain.bgm.tv/pic/user/l/icon.jpg".to_string());
                let image_grid = raw.get("images")
                    .and_then(|imgs| imgs.get("grid").or_else(|| imgs.get("medium")).or_else(|| imgs.get("large")))
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string())
                    .unwrap_or_else(|| image.clone());

                let stat_collects = raw.get("stat").and_then(|s| s.get("collects")).and_then(|v| v.as_i64()).unwrap_or(0);
                let stat_comments = raw.get("stat").and_then(|s| s.get("comments")).and_then(|v| v.as_i64()).unwrap_or(0);
                ...
                let name_cn = raw.get("name_cn").and_then(|v| v.as_str()).map(|s| s.to_string())
                    .or_else(|| {
                        raw.get("infobox")
                            .and_then(|v| v.as_array())
                            .and_then(|arr| arr.iter().find(|it| it.get("key").and_then(|k| k.as_str()) == Some("简体中文名")))
                            .and_then(|it| it.get("value"))
                            .and_then(|v| v.as_str())
                            .map(|s| s.to_string())
                    });

                let name_en = raw.get("infobox")
                    .and_then(|v| v.as_array())
                    .and_then(|arr| arr.iter().find(|it| it.get("key").and_then(|k| k.as_str()) == Some("别名")))
                    .and_then(|it| it.get("value"))
                    .and_then(|v| v.as_array())
                    .and_then(|aliases| {
                        let find_alias = |k: &str| aliases.iter()
                ...
                super::cache_put_ttl(&super::CHARACTER_DETAILS_CACHE, cache_key, 10 * 60 * 1000, out.clone());
...
async fn search_subjects(
    ...
    let keyword = q.get("keyword").cloned().unwrap_or_default().trim().to_string();
    if keyword.is_empty() {
    ...
    let list = pools
        ...
        .map(|doc| {
            ...
            let (name_cn, name_en_any, image, _image_grid, gender, _summary, popularity) =
                game::parse_character_basic_fields(doc.id, &json!({
                    "infobox": doc.infobox,
                    "summary": doc.summary,
