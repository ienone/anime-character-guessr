...
use crate::db::{DbPools, CharacterCache};
use std::sync::Arc;
...
pub fn assemble_character(cache: &CharacterCache, char_id: i64, settings: &GameSettings) -> Result<Value> {
    let char_val = cache.char_json.get(&char_id)
        .cloned()
        .ok_or_else(|| anyhow::anyhow!("Character {} not found in cache", char_id))?;
    let subjects = cache.char_subjects.get(&char_id)
        .cloned()
        .unwrap_or_default();
    let subject_infos: Vec<SubjectInfo> = subjects.into_iter().map(|r| SubjectInfo {
        id: r.id,
        role: r.role,
        stype: r.stype,
        year: r.year,
...
fn pick_candidate(pools: &Arc<DbPools>, settings: &GameSettings) -> Result<i64> {
    ...
    let sources: Vec<&Vec<(i32, i64, i64)>> = if types.len() == 4 {
        ...
        types.iter()
            .filter_map(|t| idx.by_type.get(t))
            .collect()
    };
    ...
    for bucket in sources {
        for &(year, char_id, _collects) in bucket.iter().take(top_n) {
            // Year filter
            if let Some(sy) = settings.start_year { if year < sy { continue; } }
            if let Some(ey) = settings.end_year   { if year > ey { continue; } }
            // Only include characters present in the cache
            if !cache.char_json.contains_key(&char_id) { continue; }
...
impl GameSettings {
    ...
    pub fn from_json(v: &Value) -> Self {
        Self {
            start_year: v.get("startYear").and_then(|x| x.as_i64()).map(|x| x as i32),
            end_year: v.get("endYear").and_then(|x| x.as_i64()).map(|x| x as i32),
            meta_tags: v.get("metaTags")
                .and_then(|x| x.as_array())
                .map(|arr| arr.iter().filter_map(|s| s.as_str().map(|s| s.to_string())).collect())
                .unwrap_or_default(),
            top_n_subjects: v.get("topNSubjects").and_then(|x| x.as_i64()),
            common_tags: v.get("commonTags").and_then(|x| x.as_bool()).unwrap_or(true),
            subject_tag_num: v.get("subjectTagNum").and_then(|x| x.as_u64()).unwrap_or(6) as usize,
            character_tag_num: v.get("characterTagNum").and_then(|x| x.as_u64()).unwrap_or(6) as usize,
            main_character_only: v.get("mainCharacterOnly").and_then(|x| x.as_bool()).unwrap_or(true),
...
fn assemble_payload(
    ...
    let name = char_val.get("name").and_then(|v| v.as_str()).unwrap_or("").to_string();
    ...
    let current_year = chrono::Utc::now().format("%Y").to_string().parse::<i32>().unwrap_or(2099);

    let filtered: Vec<&SubjectInfo> = subjects.iter().filter(|s| {
        if !allowed_types.contains(&s.stype) { return false; }
        if s.year <= 0 || s.year > current_year { return false; }
        if let Some(sy) = settings.start_year { if s.year < sy { return false; } }
        if let Some(ey) = settings.end_year   { if s.year > ey { return false; } }
        true
    }).collect();
    ...
    let source_tag_set: HashSet<&str> = ["原创", "游戏改", "小说改", "漫画改"].iter().cloned().collect();
    let source_tag_map: HashMap<&str, &str> = [
        ("GAL改", "游戏改"), ("轻小说改", "小说改"), ("轻改", "小说改"),
        ("原创动画", "原创"), ("网文改", "小说改"), ("漫改", "漫画改"),
        ("漫画改编", "漫画改"), ("游戏改编", "游戏改"), ("小说改编", "小说改"),
    ].iter().cloned().collect();
    ...
    for s in &sorted_subjects {
        ...
        let year = s.year;
        if year > 0 {
            if latest_appearance == -1 || year > latest_appearance { latest_appearance = year; }
            if earliest_appearance == -1 || year < earliest_appearance { earliest_appearance = year; }
        }

        if s.score > highest_rating { highest_rating = s.score; }

        let sub_name = if !s.name_cn.is_empty() { s.name_cn.clone() } else { s.name.clone() };
        ...
        if settings.common_tags {
            for tag_obj in &subject_tags {
                ...
                if tag_name.contains("20") { continue; } // skip year tags
                if let Some(&mapped) = source_tag_map.get(tag_name) {
                    *source_tag_counts.entry(mapped.to_string()).or_insert(0) += stuff_factor * tag_count;
                } else if source_tag_set.contains(tag_name) {
                    *source_tag_counts.entry(tag_name.to_string()).or_insert(0) += stuff_factor * tag_count;
                } else {
                    *raw_tags_map.entry(tag_name.to_string()).or_insert(0) += stuff_factor * tag_count;
                }
            }
        } else {
            // meta_tags mode
            for meta in &subject_meta_tags {
                ...
                if source_tag_set.contains(tag) { continue; }
            ...
            for tag_obj in &subject_tags {
                let tag_name = tag_obj.get("name").and_then(|v| v.as_str()).unwrap_or("");
                let tag_count = tag_obj.get("count").and_then(|v| v.as_i64()).unwrap_or(0);
                if tag_name.contains("20") { continue; }
                if source_tag_set.contains(tag_name) {
                    *source_tag_counts.entry(tag_name.to_string()).or_insert(0) += tag_count * stuff_factor;
                } else if let Some(&mapped) = source_tag_map.get(tag_name) {
                    *source_tag_counts.entry(mapped.to_string()).or_insert(0) += tag_count * stuff_factor;
                } else if region_tag_set.contains(tag_name) {
                    region_tags.insert(tag_name.to_string());
                } else {
                    *tag_counts.entry(tag_name.to_string()).or_insert(0) += tag_count * stuff_factor;
                }
            }
        }
    }
    ...
    if settings.common_tags {
        ...
        let mut sorted_raw: Vec<(String, i64)> = raw_tags_map.into_iter()
        ...
        for (tag, _) in &sorted_meta {
            if meta_set.len() >= settings.subject_tag_num { break; }
            if !meta_set.contains(tag) { meta_set.push(tag.clone()); }
        }
