...
pub async fn update_character_tags(
    ...
    let id = match body.get("_id").and_then(|v| v.as_i64()) {
        Some(i) => i,
        None => return (StatusCode::BAD_REQUEST, Json(json!({ "error": "Missing _id" }))).into_response(),
    };
...
pub async fn update_game_character_tags(
    ...
    let subject_id = match body.get("subjectId").and_then(|v| v.as_i64()) {
        Some(i) => i,
        None => return (StatusCode::BAD_REQUEST, Json(json!({ "error": "Missing subjectId" }))).into_response(),
    };
...
pub async fn feedback_tags(
    ...
    let id = match body.get("characterId").and_then(|v| v.as_i64()) {
        Some(i) => i,
        None => return (StatusCode::BAD_REQUEST, Json(json!({ "error": "Missing characterId" }))).into_response(),
    };
...
pub async fn bug_feedback(
    ...
    let bug_type = body.get("type").and_then(|v| v.as_str()).unwrap_or("unknown").to_string();
    ...
    let errors = body.get("errors").map(|v| v.to_string()).unwrap_or_default();
    let diagnostic_data = body.get("diagnosticData").map(|v| v.to_string()).unwrap_or_default();
